...
author: "Philippe Grosjean (phgrosjean@sciviews.org) & Guyliann Engels"
date: "`r Sys.Date()`"
output:
...
labs <- list(height = "Heigth", age = "Age", Seed = "Seed")
...
res <- sapply(levels(Loblolly$Seed), function(x) {
  sample(which(Loblolly$Seed == x), size = 1)
})
pine <- Loblolly[res,]

pine <- data.io::labelise(pine,
  label = labs, units = units)
